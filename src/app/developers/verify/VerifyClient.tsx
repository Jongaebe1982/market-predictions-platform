...
export function VerifyClient() {
  ...
  useEffect(() => {
    if (!token) {
      setState('error');
      setResult({ error: 'Missing token', message: 'No verification token provided' });
      return;
    }

    const verify = async () => {
      try {
        const res = await fetch(`/api/developer/verify?token=${token}`);
        ...
        setState('error');
        setResult({ error: 'Network error', message: 'Failed to verify. Please try again.' });
      }
    };
  ...
  if (state === 'error') {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600">{result?.message || 'Something went wrong'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="text-center py-4">
      ...
      <div className="mt-6 text-sm text-gray-500">
        <p><strong>Plan:</strong> Free ({result?.limits?.daily} requests/day)</p>
